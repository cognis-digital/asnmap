...
**asnmap** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
the suite's integration SDK. It maps any tool's JSON into a canonical **Finding** and
forwards it to the platforms that fit the **Sanctions - ownership - finance** domain.
...
Once `asnmap` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
asnmap ... --format json | cognis-connect emit --to misp --url $URL --token $TOK   # MISP event
asnmap ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
asnmap ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
```
...
works whatever `asnmap` outputs:

```python
from cognis_connect import normalize, misp
findings = [normalize(rec, source="asnmap") for rec in records]   # records = your JSON output
print(misp.to_event(findings))
```
