"""Native cognis-connect emit for asnmap — forward findings to any platform.

Maps asnmap's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    asnmap ... --format json | asnmap-emit --to stix
    asnmap-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    """Tailor a asnmap record toward the Finding contract before normalize().

    Default is a pass-through (normalize() already aliases common fields/indicators).
    Tool-specific enrichment is appended below by the build process when available.
    """
    return rec
